...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
     ...
     *
     * @throws BeansException        异常 当bean工厂无法初始化的时候抛出
     * @throws IllegalStateException 异常 当已经初始化且不支持多次刷新的时候抛出
     */
    @Override
    public void refresh() throws BeansException, IllegalStateException {
        ...
        synchronized (this.startupShutdownMonitor) {
            ...
            // 把拿到的 beanFactory 做一些准备，这里其实没啥逻辑，同学们感兴趣的可以看下
            // 但是这个方法也是一个 protected 的方法，
            // 也就是说我们如果实现自己的 spring 启动类/或者 spring 团队需要写一个新的 spring 启动类的时候
            // 是可以在 beanFactory 获取之后做一些事情的，算是一个钩子
            prepareBeanFactory(beanFactory);

            try {
                // 允许容器的子类去注册 postProcessor  ，钩子方法
                // 这也是一个钩子，在处理 beanFactory 前允许子类做一些事情
                ...
                // 实例化并且调用 factoryPostProcessor 的方法，
                // 我们 @Component 等注解的收集处理主要就是在这里做的
                // 有一个 ConfigurationClassPostProcessor 专门用来做这些注解支撑的工作
                // 这里的逻辑之前也讲过了
                // 那么其实我们可以说，到这里为止，我们的 beanDefinition 的收集（注解/xml/其他来源...）
                // 注册（注册到 beanFactory 的 beanDefinitionMap、beanDefinitionNames）容器
                // 工作基本就全部完成了
                invokeBeanFactoryPostProcessors(beanFactory);

                // 从这里开始，我们就要专注bean的实例化了
                // 所以我们需要先实例化并注册所有的 beanPostProcessor
                // 因为 beanPostProcessor 主要就是在bean实例化过程中，做一些附加操作的（埋点）
                ...
                // 清除临时的 ClassLoader，实例化所有的类（懒加载的类除外）

                // !!!实例化所有的、非懒加载的单例bean
                finishBeanFactoryInitialization(beanFactory);

                // 初始化容器的生命周期事件处理器，（默认使用DefaultLifecycleProcessor），调用扩展了 SmartLifecycle 接口的start方法
     ...
     * 这允许在某些应用上下文实现中注册特殊的 BeanPostProcessors 等。
     *
     * @param beanFactory 应用环境下的 Bean 工厂
     */
