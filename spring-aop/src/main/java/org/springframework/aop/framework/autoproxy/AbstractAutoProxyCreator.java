...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
		...
		//beanName是否为空
		//若beanName不为空, 而且targetSourcedBeans不包含这个beanName的话
		//targetSourcedBeans 只是供类内部使用的缓存
		if (!StringUtils.hasLength(beanName) || !this.targetSourcedBeans.contains(beanName)) {
			//advisedBeans保存了所有已经做过动态代理的Bean
			// 如果被解析过则直接返回
			if (this.advisedBeans.containsKey(cacheKey)) {
				//返回为null就会继续执行doCreateBean的操作
				return null;
			}
		...
		if (targetSource != null) {
			if (StringUtils.hasLength(beanName)) {
				//targetSourcedBeans 只在这里进行添加
				//这里添加的 正是getCustomTargetSource()方法所返回的TargetSource
				//所对应的beanName
				//所以, 可以肯定的是, 只有用户自定义的targetSource的BeanName才会被添加
			...
			//成功创建代理之后, 就将代理的类型给放入到proxyTypes缓存中, 便于后续的获取
	...
	protected Object getCacheKey(Class<?> beanClass, @Nullable String beanName) {
		//如果beanName不为空
		if (StringUtils.hasLength(beanName)) {
			//判断当前的bean是否是FactoryBean
			//如果是 键位 = & +beanName
			//如果不是直接将beanName返回
			return (FactoryBean.class.isAssignableFrom(beanClass) ?
					BeanFactory.FACTORY_BEAN_PREFIX + beanName : beanName);
		}
		//如果传入的beanName为空, 则直接以BeanClass对象返回
		else {
			return beanClass;
		}
	}
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		// 如果beanName对应的值是false 则直接返回bean
		if (Boolean.FALSE.equals(this.advisedBeans.get(cacheKey))) {
			return bean;
		}
		// 是不是基础的bean 是不是需要跳过的
		// 针对非Singleton的Aspect来讲, 是有必要再次进行判断的, 毕竟其是不需要经过缓存的
		/*
		...
		//getAdvicesAndAdvisorsForBean()验证当前的Bean匹不匹配Advisors

		// 为目标 bean 查找合适的通知器
		// 这个方法将得到所有的可用于拦截当前 bean 的 advisor、advice、interceptor。
		Object[] specificInterceptors = getAdvicesAndAdvisorsForBean(bean.getClass(), beanName, null);
		//只有筛选出匹配的Bean, 才会去调用createProxy()方法
		/*
	...
	protected TargetSource getCustomTargetSource(Class<?> beanClass, String beanName) {
		...
		//查看是否存在用户自定义的TargetSourceCreator列表 -> customTargetSourceCreators
		if (this.customTargetSourceCreators != null &&
				this.beanFactory != null && this.beanFactory.containsBean(beanName)) {
			//若存在, 则依次调用他们的getTargetSource()方法 去获取TargetSource
			for (TargetSourceCreator tsc : this.customTargetSourceCreators) {
				TargetSource ts = tsc.getTargetSource(beanClass, beanName);
				//直到其中一个TargetSource返回不为空
				//则证明已经获取到了TargetSource
				if (ts != null) {
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName,
		...
		// 将当前的AnnotationAwareAspectJAutoProxyCreator 对象的属性赋值给ProxyFactory对象
		proxyFactory.copyFrom(this);
		/*
		 *  处理 proxyTargetClass 属性
		 *  如果希望使用 CGLIB 来代理接口，可以配置 proxy-target-class="true",
		 *  这样不管有没有接口，都使用 CGLIB 来生成代理：
		 *  <aop:config proxy-target-class="true"></aop:config>
		*/
		if (!proxyFactory.isProxyTargetClass()) { // false 使用JDK进行动态代理
			// shouldProxyTargetClass()方法判断是否使用目标类（即CGLIB这种方式）进行代理
			// 返回true则使用CGLIB进行代理
