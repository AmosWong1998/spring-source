...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
    ...
    public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
        ...
        for (BeanPostProcessor processor : getBeanPostProcessors()) {
            // bean构造完成之后调用，责任链模式 如果哪个处理器处理之后返回空值 则返回result
    ...
    protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
        ...
        // 获取容器中的BeanDefinition实例，当前的beanDefinition实例可能有parent属性
        // 这里需要封装到RootBeanDefinition里面
        ...
        // 既然要创建Bean实例 获取class对象是不可或缺的
        Class<?> resolvedClass = resolveBeanClass(mbd, beanName);
        // 获取到的resolvedClass不为空，并且bean没有class对象, 却有className的时候, 也就是XML方式
        // 注解方式不会进入if语句，因为BeanDefinition里面已经存在BeanClass了
        ...
        // 校验和准备Bean中的方法覆盖
        try {
            // 判断BeanDefinition里是否有定义方法的覆盖

            // 这里对beanDefinition中的MethodOverrides做一些准备
            // 主要是梳理一下所有重写方法（xml中 <replaced-method><lockup-method>标签对应的属性）
    ...
    protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
        ...
        if (earlySingletonExposure) {
            ...
            // 将实例放入ObjectFactory里面，然后调用addSingletonFactory将相关ObjectFactory实例添加到三级缓存中
        ...
        if (earlySingletonExposure) {
            ...
            if (earlySingletonReference != null) {
                ...
                // 即使自己最终会被包装（代理）
                // dependentBeanMap记录着每个依赖与此Bean的Bean实例集合
                // 当发生循环引用时不允许新创建实例对象
    ...
    protected Object getEarlyBeanReference(String beanName, RootBeanDefinition mbd, Object bean) {
        ...
        if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
            for (BeanPostProcessor bp : getBeanPostProcessors()) {
                ...
                if (bp instanceof SmartInstantiationAwareBeanPostProcessor) {
                    SmartInstantiationAwareBeanPostProcessor ibp = (SmartInstantiationAwareBeanPostProcessor) bp;
                    // 调用getEarlyBeanReference()对bean实例包装处理
    ...
    protected Object applyBeanPostProcessorsBeforeInstantiation(Class<?> beanClass, String beanName) {
        for (BeanPostProcessor bp : getBeanPostProcessors()) {
            if (bp instanceof InstantiationAwareBeanPostProcessor) {
                ...
                // 逐个去调用postProcessBeforeInstantiation()方法
                // result!=null 证明实例已经被自定义的后置处理器创建了 不需要后续操作了 直接return;
                // 并且不会执行后续的#doCreateBean()方法
    ...
    protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
        ...
        // 该 bean 是多例的，并且已经创建过一次了 可以直接调用构造方法解析
        if (resolved) {
            // 自动注入，调用构造函数自动注入
    ...
    protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
        ...
        /* postProcessAfterInstantiation:在设置属性前去修改Bean状态 */

        // 不是Spring内部的bean 并且容器中已经注册了InstantiationAwareBeanPostProcessors
        // 方法作为属性赋值的前置检查条件，在属性赋值之前执行，能够影响是否进行属性赋值
        if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
            for (BeanPostProcessor bp : getBeanPostProcessors()) {
                if (bp instanceof InstantiationAwareBeanPostProcessor) {
                    // ibp 在属性注入之前 有最后一次修改属性值的机会
                    InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
                    // 责任链模式 调用这些后置处理器的postProcessAfterInstantiation方法
                    if (!ibp.postProcessAfterInstantiation(bw.getWrappedInstance(), beanName)) {
                        // 框架内部后续的属性填充步骤不用进行了，已经被用户自定义的程序进行接管了
        ...
        /* 注入属性到PropertyValues中（按照名字或者类型装配） */

        int resolvedAutowireMode = mbd.getResolvedAutowireMode();
        // 这里使用注解方式@Autowired 不会再进行解析属性了，因为前面已经解析完属性并将其放入到InjectMetadata中了
        // 只有XML 方式才会进入这个if语句
        ...
        if (hasInstAwareBpps) {
            ...
            /* postProcessProperties 对解析完但是没设置完的属性进行再处理 */
            for (BeanPostProcessor bp : getBeanPostProcessors()) {
                if (bp instanceof InstantiationAwareBeanPostProcessor) {
                    InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
                    // 在这里会对@Autowired标记的属性进行依赖注入
                    // 注意是AutowiredAnnotationBeanPostProcessor这个后置处理器
    ...
    protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
        if (System.getSecurityManager() != null) {
            ...
            // 这里调用的是Group1中的三个Bean开头的Aware(BeanNameAware、BeanClassLoaderAware、BeanFactoryAware)
            invokeAwareMethods(beanName, bean);
        }

        Object wrappedBean = bean;
        // 不是Spring内置的bean，调用beanPostProcessors去处理
        if (mbd == null || !mbd.isSynthetic()) {
            /**
             * 这里调用的是Group2中的几个Aware，EnvironmentAware、EmbeddedValueResolverAware、ApplicationContextAware
