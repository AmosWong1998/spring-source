...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
    ...
    protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
        ...
        // 看这个 bean 是否已经实例化了，如果已经实例化了可以直接拿到
        ...
        if (sharedInstance != null && args == null) {
            ...
            // 通过这个 bean 实例获取用户真正需要的bean实例
            // 有点绕，其实这里主要是处理当前 bean 实现了 FactoryBean 接口的情况的
        ...
        // 若scope为prototype或者单例模式但是缓存中还不存在bean
            ...
            // A->B->A 在创建第二个A的时候 由于第一个A还没有创建成功 会被保存到
            // ThreadLocal<Object> prototypesCurrentlyInCreation 中
            // 第二个A来到这 判断当前Bean正在被创建 此时就会抛出异常
            ...
            if (parentBeanFactory != null && !containsBeanDefinition(beanName)) {
                // 如果父容器不为空且当前容器没有这个 beanName 对应的 beanDefinition
                // 则尝试从父容器获取（因为当前容器已经确定没有了）
                // 下面就是调用父容器的getBean了
                // Not found -> check parent.

                // 主要针对FactoryBean，将Bean的&重新加上
                String nameToLookup = originalBeanName(name);
                // 如果parent容器依旧是 AbstractBeanFactory 的实例
            ...
            if (!typeCheckOnly) { // typeCheckOnly 默认传入为false
                // 如果不是仅仅做类型检查则是创建bean
            ...
            try {
                ...
                if (dependsOn != null) {
                    ...
                    for (String dep : dependsOn) {
                        ...
                        // depends-on 会指明 bean 创建的顺序，如果显式循环依赖，Spring就不知道先创建哪个了
				...
				// 这里开始真正创建bean实例的流程了
				if (mbd.isSingleton()) {
                    // 如果是单例的 bean（当然我们启动的时候会实例化的也就是单例bean了），这里会进行创建
                    // 注意这里也是一个 getSingleton 方法，跟之前那个 getSingleton 方法差不多，不过这里是
                    // 如果获取不到就会使用这个 lambda 的逻辑创建一个，
                    // 也就是说我的的createBean方法是真正创建bean实例的方法，这里我们之后会重点看
                    // 这里使用了一个匿名内部类，创建Bean实例对象，并且注册给所依赖的对象
                    // 匿名内部类为 ObjectFactory 主要实现了 ObjectFactory#getObject() 方法的逻辑
    ...
    protected boolean isPrototypeCurrentlyInCreation(String beanName) {
        // ThreadLocal<Object> prototypesCurrentlyInCreation 存储本线程正在创建的实例
    ...
    protected void afterPrototypeCreation(String beanName) {
        ...
        if (curVal instanceof String) {
            // 将 prototypesCurrentlyInCreation 缓存给清除掉
    ...
    protected RootBeanDefinition getMergedLocalBeanDefinition(String beanName) throws BeansException {
        ...
        // 获取之前已经合并好了的BeanDefinition实例
        RootBeanDefinition mbd = this.mergedBeanDefinitions.get(beanName);
        // mbd!=null 并且 合并标志位为false 直接返回即可
        if (mbd != null && !mbd.stale) {
            return mbd;
        }
        // stale=true 会重新合并子类和父类的BeanDefinition
            ...
            synchronized (this.mergedBeanDefinitions) {
                if (!this.alreadyCreated.contains(beanName)) {
                    ...
                    /**
                     ...
                     这两个BeanDefinition的实例 会被合并成同一个来使用
                     */
                    // 给BeanDefinition 设置上一个需要清除的标志符，让后续从容器获取BeanDefinition的时候
                    // 能够重新合并父类和子类的BeanDefinition 这样为了防止BeanDefinition的原数据被改动了
                    // 还是按照原来的策略去创建
    ...
    protected Object getObjectForBeanInstance(
        ...
        if (object == null) {
            ...
            if (mbd == null && containsBeanDefinition(beanName)) {
                // 获取beanName在容器中对应的BeanDefinition实例
                mbd = getMergedLocalBeanDefinition(beanName);
            }
            // synthetic用来标志 一个类是由内部机制生成的一个类
            // bean的这个属性为true 代表是Spring容器内部生成的Bean 不允许用户或者第三方去改动
            boolean synthetic = (mbd != null && mbd.isSynthetic());
            // 缓存里面没有，需要自行创建FactoryBean
